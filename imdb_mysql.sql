...
-- update table data
update user_review
set user_reviews='ragul'
where id=1;
-- delete table data
delete from user_review where id=1;
...
-- get table data
 select * from imgs;
--  join multiple table data
 select movie_id,image,movie_name,movie_year,movie_rating,actor_name,movie_role_one,movie_role_two
 from movies m 
 join imgs i on m.id=i.image_id
 join skills s on m.id=s.skillid
 join roles r on m.id=r.role_id;
